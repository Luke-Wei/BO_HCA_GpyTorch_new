...
def Experiments(repet_time):
    ...
    for i in range(iteration):
        ...
        # Use standard acquisition functions without any modifications
        if algorithm == "EI":
            ...
            # Sample once from posterior and select minimum point
            mu_ts = mu_g
            sigma_ts = sigma_g
            # Ensure positive standard deviation
            eps = 1e-9
            std_clipped = np.maximum(sigma_ts, eps)
            # Single sample from the posterior
            y_sample = np.random.normal(loc=mu_ts, scale=std_clipped)
            # Select the point with the minimum sampled value
            min_index = np.argmin(y_sample)
            x_new = X_[min_index]
        elif algorithm == "UCB":
            # Standard UCB with typical exploration constant (2.0)
            x_new = UCB(mu_g, sigma_g, X_, const=2.0)
        else:
            # Default to EI
