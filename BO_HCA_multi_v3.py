...
class HeteroscedasticLikelihood(gpytorch.likelihoods.MultitaskGaussianLikelihood):
    ...
    def forward(self, function_samples, **kwargs):
        ...
        if 'credits' in kwargs and kwargs['credits'] is not None:
            ...
            # 仅调整任务0的噪声，任务1（credit本身）的噪声保持不变
            adjusted_noise = noise_diag.clone()
            # 增强credit对噪声的影响，平方关系使得高credit样本的噪声更低
            adjusted_noise[:, 0] = adjusted_noise[:, 0] / (credits * credits)  # 使用平方关系增强影响
...
#############################################
# 训练多任务 GP 模型（利用 gpytorch 训练）- 修改使用新的似然函数
#############################################
...
#############################################
# 利用训练好的模型做预测，返回均值和标准差（多任务，每列对应一个任务）- 修改以传递credit
#############################################
...
def Experiments(repet_time):
    ...
    for i in range(iteration):
        ...
        if algorithm == "EI":
            ...
            # 首先获取多任务预测
            mu_pred_temp, sigma_pred_temp = gp_predict(model, likelihood, X_temp, credits=None)
            
            # 提取任务0（目标函数）的预测值
            mu_objective = mu_pred_temp[:, 0]  # 只取第一列（目标函数）
            sigma_objective = sigma_pred_temp[:, 0]  # 只取第一列（目标函数）
            
            # 直接使用目标函数的预测值，而不是多任务模型
            # 重复采样n_points次
            n_points = 5
            eps = 1e-9
            mu = mu_objective.reshape(-1, 1)
            sigma = sigma_objective.reshape(-1, 1)
            ...
            print("select UCB")
            # UCB已经是最小化设置，增大探索系数以增强不确定性影响
            x_new = UCB(mu_g, sigma_g, X_, const=3.5)  # 使用更大的探索系数
        else:
            # 默认使用 EI
            y_best = np.min(Y_update[:, 0])  # 最小化问题
            x_new = EI(mu_g, sigma_g, X_, y_best)
        ...
        if heter:
            ...
            Z_val = np.min(Y_update[:, 0])  # 修改为最小值
            eps = 1e-6
        ...
        # 传递当前数据点的credit用于调整预测噪声
        current_credits = Y_update[:, 1]
        mu_pred, sigma_pred = gp_predict(model, likelihood, X_, credits=current_credits)  # 修改：使用当前credits进行预测
        mu_g = mu_pred[:, 0]
        sigma_g = sigma_pred[:, 0]
...
# 参数解析及主函数入口
#############################################
parser = argparse.ArgumentParser(description='HCA-Improved with Multioutput GP and Heteroscedastic Noise (gpytorch)')
parser.add_argument("-f", "--fff", help="a dummy argument to fool ipython", default="1")
parser.add_argument('-t', '--time', type=str, help='Date of the experiments', default='HCA_multi_v318')
