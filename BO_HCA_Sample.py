    ...
    for i in range(iteration):
        ...
        if heter and i > 0:  # 第一轮迭代还没有credit信息，从第二轮开始使用非均匀采样
            ...
            X_ = generate_candidates_with_credit_bias(
                ...
                smooth_power=smooth_power,  # 新增：credit平滑指数
                min_prob=0.05,             # 新增：最小概率阈值
                use_repulsion=True,        # 新增：启用区域排斥
                repulsion_strength=repulsion_strength  # 新增：排斥强度
            )
...
parser.add_argument('-t', '--time', type=str, help='Date of the experiments', default='HCA_Sample_v318')
