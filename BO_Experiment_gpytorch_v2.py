...
def Experiments(repet_time):
    ...
    for i in range(iteration):
        ...
        # =============== (2) 评估新点，更新数据 ===============
        y_new = f(x_new)  # 目标函数值
        # baseline：credit 始终为 1，不做任何 HCA 更新
        credit_new = 1.0
...
# 参数解析及主函数入口
#############################################
parser = argparse.ArgumentParser(description='Baseline: No HCA, with Multioutput GP (gpytorch)')
parser.add_argument("-f", "--fff", help="a dummy argument to fool ipython", default="1")
parser.add_argument('-t', '--time', type=str, help='Date of the experiments', default='HCA_multi_baseline_v2')
