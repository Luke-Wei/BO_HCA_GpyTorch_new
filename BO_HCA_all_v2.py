...
def Experiments(repet_time):
    ...
    Budget = range(1, N_total + 1) # Budget from 1 to N_total
    file_base = "_".join([
        date, func, algorithm, f"heter{heter}",
        f"noise{dy}", f"init{n_sample}", f"iter{iteration}", f"dim{dimension_x}"
    ])
    imagefile = os.path.join("Images", file_base)
    resultfile = os.path.join("Results", file_base)
    ...
    plt.savefig(f"{imagefile}_run{repet_time}.pdf", format="pdf")
    plt.close()

    # Saving data
    # Use append mode 'ab' for saving across multiple repetitions if needed, or 'wb'/'w' for single run overwrite
    # For multi-run append, headers would be needed or careful post-processing
    # Let's assume we save each run separately or handle aggregation later. Using 'w' (via np.savetxt)
    save_mode = "w" # or 'a' if appending rows from multiple runs to the same file

    # Prepare data for saving - ensure all are 2D arrays N_total x k
    results_to_save = {
        "_D_update.csv": D_update,                  # (N_total, D) - All evaluated points
        "_Y_update.csv": Y_update,                  # (N_total, 2) - [Objective, Credit] for evaluated points
        "_Minimizer_Seq.csv": minimizer_all,        # (N_total, D) - Sequence of best points found
        "_Minimum_True_Seq.csv": minimum_true_at_minimizers, # (N_total, 1) - True value at best points
        "_GAP_Seq.csv": GAP,                        # (N_total, 1) - Gap |f(x_best) - f*| sequence
        "_SimRegret.csv": SimRegret,                # (N_total, 1) - Simple Regret sequence
        "_CumRegret.csv": CumRegret,                # (N_total, 1) - Cumulative Regret sequence
        "_AvgRegret.csv": AvgRegret,                # (N_total, 1) - Average Regret sequence
        "_InstantRegret.csv": Regret,               # (N_total, 1) - Instantaneous Regret sequence
        "_Time_Per_Iter.csv": TIME_RE.T,            # (iteration, 1) - Time per iteration
        "_Run_Info.txt": None                       # Placeholder for text info
    }
    if x_star is not None:
        results_to_save["_xGAP_Seq.csv"] = xGAP     # (N_total, 1) - Distance ||x_best - x*|| sequence


    for suffix, data in results_to_save.items():
        filepath = f"{resultfile}{suffix}"
        if suffix == "_Run_Info.txt":
             with open(filepath, save_mode) as f_out:
                 f_out.write(f"Run: {repet_time}\n")
                 f_out.write(f"Total Time: {end_all:.4f}\n")
                 f_out.write(f"Final RMSE (Best Found vs f*): {rmse:.4f}\n")
                 f_out.write(f"Args: {args}\n")
        elif data is not None:
             # Ensure data is numpy array before saving
             if isinstance(data, list):
                 data = np.array(data)
             # Save with header (optional, but good practice)
             header_str = f"Run_{repet_time}" # Basic header
             # Or create more descriptive headers based on data shape
             # np.savetxt(filepath, data, delimiter=",", header=header_str, comments='') # Use comments='' to avoid '#' in header
